...
do
	if [ -f "$1.tensiometer.${k}.data.json" ]; then
		...
		split $1.tensiometer.${k}.data.json -a 1 -d $1.tensiometer.${k}.data_split_
		...
		if [ $NFILE -gt 1 ];
		then
			echo "]" >> $1.tensiometer.${k}.data_split_0
		fi
		mv $1.tensiometer.${k}.data_split_0 $1.tensiometer.${k}.data_split_0.json

		for (( i = 1; i < $NFILE; i++ ))
		do
			sed -i '1s/^./[/' $1.tensiometer.${k}.data_split_${i}
			if [ $i -lt $(( $NFILE-1 )) ];
				then
					echo "]" >> $1.tensiometer.${k}.data_split_${i}
				fi	
			mv $1.tensiometer.${k}.data_split_${i} $1.tensiometer.${k}.data_split_${i}.json
		done
	else
